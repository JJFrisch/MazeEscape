...
public partial class World2CampaignLevelPage : ContentPage
{
    ...
    public World2CampaignLevelPage(CampaignLevel level)
    {
    ...
    public async void CompletedMaze()
    {
        ...
        try
        {
            ...
            {
                sent = true;
                var result = await this.ShowPopupAsync(new World2CampaignMazeFinishedPopupPage(TotalTime, numberOfMoves, Level, coinsEarned), CancellationToken.None);
                if (result == "Retry")
                {
                ...
                else if (result == "Next Level")
                {
                    ...
                    { // Any variables that may be changed
                        ...
                        await App.PlayerData.World2_LevelDatabase.SaveLevelAsync(next_level);
                    };
                    await Navigation.PushAsync(page);
                }
            }
